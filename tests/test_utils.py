...
class TestJinjaUtils(TestHelper):
    ...
    def test_jinja_keyword_gen_names(self):
        ll = utils.JinjaUtils._jinja_keyword_gen_names('prefix', 1)
        assert len(ll) == 1
        assert 'prefix' in ll

        ll = utils.JinjaUtils._jinja_keyword_gen_names('prefix', 2)
        assert len(ll) == 2
        assert 'prefix-0001' in ll
        assert 'prefix-0002' in ll
    ...
    def test_self_signed_cert_gen(self):
        ...
        assert subject.O == org  # noqa (skip pep8)
